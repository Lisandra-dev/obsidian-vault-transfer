...
export class SettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If set to true, the folder will be created if it does not exist. Usefull if using a date variable."
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				"If set to true, the folder structure of the original file will be recreated in the new vault."
			)
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.recreateTree).onChange(async (value) => {
		...
		if (this.plugin.settings.recreateTree) {
			new Setting(containerEl)
				...
				.setDesc(
					"Removes the specified folders from the output path, if present. Separate folders by using a comma or a new line. Names are case insensitive."
				)
				.addTextArea((text) =>
					text
			...
			for (const override of this.plugin.settings.overridedPath || []) {
				new Setting(containerEl)
					...
					.setDesc(
						"Override a part or totally the path for another one. You can use date variable here, like {{YYYY-MM-DD}}. Support regex if encapsulated in /regex/"
					)
					.addText((text) =>
						text
					...
					.addExtraButton((button) =>
						button
							...
							.setTooltip("Delete")
							.onClick(() => {
								this.plugin.settings.overridedPath =
									this.plugin.settings.overridedPath.filter((o) => o != override);
								this.display();
							})
					);
			}
		...
		if (this.plugin.settings.deleteOriginal) {
			new Setting(containerEl)
				...
				.setDesc(
					"If set to true, the original file will be moved to the system trash. Otherwise, it will be moved to the vault trash."
				)
				.addToggle((toggle) =>
					toggle
			...
			new Setting(containerEl)
				.setName("Create link")
				.setDesc(
					"Add a link to the new file in the new vault to the current note. If set to false, the file will be left unchanged."
				)
				.addToggle((toggle) =>
					toggle.setValue(this.plugin.settings.createLink).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setHeading()
			.setDesc(
				"The date variable to use for the output folder or in override path. For folder, will use the folder note (inside, outside, or named index) data."
			);

		new Setting(containerEl)
			.setName("Date Variable")
			.setDesc(
				"The date variable to use for the output folder. If set to frontmatter, you can specify the key. If set to creation or modification, the date will be used."
			)
		...
		if (this.plugin.settings.dateVariable.type == "frontmatter") {
			...
			new Setting(containerEl)
				...
				.setDesc(
					"The date to use if the frontmatter date is not set. Can be the last modification time or the file creation file."
				)
				.addDropdown((dropdown) =>
					dropdown
						.addOptions({
		...
		new Setting(containerEl)
			.setName("Overwrite")
			.setDesc(
				"If set to false, the file will be skipped if it already exists in the other vault."
			)
			.addToggle((toggle) =>
